...
# CARGA DE MODELOS ML
# =========================================================
# Prefijos válidos y a qué métrica interna corresponden
PREFIJOS_METRICAS = {
...
def cargar_modelos_ml():
    """
    Carga todos los .pkl de la carpeta models/.
    Devuelve dict: { "goles_local": [modelo1, modelo2, ...], ... }
    ...
    for archivo in archivos:
        ...
        try:
            ...
            # --- Caso 1: ensemble (diccionario con múltiples métricas) ---
            if isinstance(obj, dict):
                for clave, valor in obj.items():
                    ...
                    if isinstance(valor, list):
                        ...
                        # valor es directamente un modelo
                        try:
                            valor.predict
            ...
            # --- Caso 2: modelo individual (tiene .predict) ---
            else:
                try:
                    obj.predict  # comprueba que es un modelo
                    # Detectar métrica por nombre de archivo
            ...
            pass  # Si un pkl falla, lo ignoramos silenciosamente
 
    return modelos
 
 
def calcular_ma(df, col, ventana):
    """Calcula media móvil de los últimos N partidos de una columna."""
...
def construir_features_ml(df_local, df_visit, es_local, jornada, pos_rival):
    ...
    # Combinar local y visitante ordenados por fecha
...
def predecir_ml(modelos_ml, feats_local, feats_visit):
    """
    Lanza predicciones ML para local y visitante.
    Devuelve dict con las predicciones promediadas de todos los modelos disponibles.
    """
    if not modelos_ml:
        return None
    ...
    # Métricas que corresponden al equipo local
    metricas_local = {
    ...
    # Métricas que corresponden al equipo visitante
    metricas_visit = {
    ...
    # Calcular partido si tenemos local y visitante
    pares = [
...
def combinar_metrica_ml(metricas_metrica, pred_ml, jornada):
    """
    Combina rama métrica y rama ML con pesos según jornada.
    """
    if pred_ml is None:
        return metricas_metrica, False
 
    # Pesos: más peso a ML conforme avanza la temporada
    ...
    for k_met, v_met in metricas_metrica.items():
        # Buscar clave equivalente en ML
        ml_val = None
...
# =========================================================
# CLASIFICACIÓN DESDE LIBRO HISTÓRICO
# =========================================================
...
if n_modelos > 0:
    ...
    st.sidebar.info("ℹ️ Sin modelos ML. Usando solo rama métrica.")
...
try:
    ...
    if st.button("📊 GENERAR ANÁLISIS"):
        ...
        try:
            ...
            if not id_historico:
                st.error("❌ No se encontró el ID del libro 'HISTORICO DE PREDICCIONES'.")
                st.stop()
            ...
            clasif = obtener_clasificacion_desde_historico(
                id_historico, PESTANA_CLASIFICACION, jornada_clasificacion
            )
            ...
            if local_info.empty:
                st.error(f"❌ No se encontró '{nombre_local_clasif}' en la clasificación.")
                st.info(f"Equipos disponibles: {clasif['EQUIPO'].tolist()}")
                st.stop()
 
            if visit_info.empty:
                st.error(f"❌ No se encontró '{nombre_visit_clasif}' en la clasificación.")
                st.info(f"Equipos disponibles: {clasif['EQUIPO'].tolist()}")
                st.stop()
