...
def cargar_modelos_ml():
    ...
    Soporta tanto archivos individuales como un único diccionario (ej. modelo_xgboost(1).pkl).
    """
    modelos = {}
    carpeta = "models"

    if not os.path.exists(carpeta):
        st.sidebar.warning(f"No se encontró la carpeta '{carpeta}'")
        return modelos
    ...
    for archivo in archivos:
        ...
        try:
            ...
            # Caso 1: el objeto es un diccionario (ensemble)
            if isinstance(obj, dict):
                for clave, valor in obj.items():
                    metrica_interna = PREFIJOS_METRICAS.get(clave.upper())
                    if metrica_interna is None:
                        continue
                    # El valor puede ser un modelo, una lista de modelos, o una lista de dicts
            ...
            # Caso 2: objeto con .predict (modelo individual)
            elif hasattr(obj, "predict"):
                # Inferir métrica por el nombre del archivo
                nombre_upper = archivo.replace(".pkl", "").upper()
            ...
            st.sidebar.warning(f"Error cargando {archivo}: {e}")

    return modelos

# ---------------------------------------------------------
# (El resto de funciones: calcular_ma, construir_features_ml,
#  predecir_ml, combinar_metrica_ml, etc. se mantienen igual)
# ---------------------------------------------------------
...
# =========================================================
# DETECCIÓN DE COLUMNAS Y CARGA DE DATOS
# =========================================================
