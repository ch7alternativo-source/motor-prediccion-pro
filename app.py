...
# Logo público (ajusta la ruta si es necesario)
LOGO_URL = "https://raw.githubusercontent.com/ch7alternativo-source/motor-prediccion-pro/main/logo2.png"

def get_logo_base64(url):
    """Convierte imagen de URL a base64 para el splash"""
    try:
        r = requests.get(url)
        r.raise_for_status()
        return base64.b64encode(r.content).decode()
    except:
        # Fallback: emoji si no carga
        return ""

LOGO_B64 = get_logo_base64(LOGO_URL)

# =========================================================
# CSS GLOBAL (modo oscuro suave, opcional)
# =========================================================
st.markdown("""
<style>
...
# =========================================================
# CONEXIÓN A GOOGLE SHEETS (necesario para login)
# =========================================================
...
# =========================================================
# SPLASH SCREEN (FULLSCREEN CON FADE)
# =========================================================
if "splash_done" not in st.session_state:
    st.session_state.splash_done = False

if not st.session_state.splash_done and LOGO_B64:
    splash_html = f"""
    <style>
    .splash {{
        position: fixed;
    ...
    <div class="splash">
        ...
        <div style="color:#aaaaaa; margin-top:15px; font-size:1.2rem;">by Chiquicuenca</div>
    </div>
    """
...
def check_user(user, pwd):
    try:
        ...
        for row in data:
            if row[0].strip() == user.strip() and row[1].strip().replace(".0", "") == pwd.strip():
                return True
        return False
    except Exception:
        return False
...
if not st.session_state.autenticado:
    # Logo y título en login
    if LOGO_B64:
        st.image(LOGO_URL, width=180)
    else:
        st.markdown("# COMBIBET PRO")
...
# =========================================================
# A PARTIR DE AQUÍ: TODO EL CÓDIGO ORIGINAL (sin modificaciones funcionales)
# =========================================================
...
# ---------------------------------------------------------
# FUNCIONES AUXILIARES (métricas, ML, etc.)
# ---------------------------------------------------------
...
# ---------------------------------------------------------
# DETECCIÓN DE COLUMNAS Y NORMALIZACIÓN
# ---------------------------------------------------------
...
def mapear_columnas(df):
    mapeo = {}
    columnas_buscar = {
        "GOL FAVOR": ["gol favor", "goles favor", "gf", "goles_marcados", "gol local", "goles local", "goles locales", "goles a favor", "gf local"],
        "GOL CONTRA": ["gol contra", "goles contra", "gc", "goles_recibidos", "gol visitante", "goles visitante", "goles visitantes", "goles en contra", "gc visitante"],
        "REMATES TOTALES FAVOR": ["remates totales favor", "remates totales f", "total shots for", "remates favor", "remates totales local", "remates totales locales", "total shots local"],
        "REMATES TOTALES CONTRA": ["remates totales contra", "remates totales c", "total shots against", "remates contra", "remates totales visitante", "remates totales visitantes", "total shots visitante"],
        "REMATES PUERTA FAVOR": ["remates puerta favor", "remates a puerta favor", "remates puerta f", "shots on target for", "remates puerta local", "remates a puerta local", "shots on target local", "tiros a puerta local", "tiros a puerta favor", "shots on target", "tiros a puerta", "remates a puerta", "remates al arco", "remates al arco local", "tiros portería local", "sot local", "sot favor", "remates al arco favor", "rematrs puerta favor", "rematrs puerta local", "rematrs a puerta favor"],
        "REMATES PUERTA CONTRA": ["remates puerta contra", "remates a puerta contra", "remates puerta c", "shots on target against", "remates puerta visitante", "remates a puerta visitante", "shots on target visitante", "tiros a puerta visitante", "tiros a puerta contra", "remates a puerta visitante", "sot visitante", "sot contra", "remates al arco visitante", "rematrs puerta contra", "rematrs puerta visitante", "rematrs a puerta contra"],
        "PARADAS FAVOR": ["paradas favor", "paradas f", "saves for", "paradas realizadas", "paradas local", "paradas locales", "saves local"],
        "PARADAS CONTRA": ["paradas contra", "paradas c", "saves against", "paradas visitante", "paradas visitantes", "saves visitante"],
        "CORNERES FAVOR": ["corneres favor", "corners favor", "corner favor", "corneres local", "corners local", "córners local"],
        "CORNERES CONTRA": ["corneres contra", "corners contra", "corner contra", "corneres visitante", "corners visitante", "córners visitante"],
        "TARJETAS AMARILLAS FAVOR": ["tarjetas amarillas favor", "amarillas favor", "yellow cards for", "tarjetas amarillas local", "amarillas local"],
        "TARJETAS AMARILLAS CONTRA": ["tarjetas amarillas contra", "amarillas contra", "yellow cards against", "tarjetas amarillas visitante", "amarillas visitante"],
